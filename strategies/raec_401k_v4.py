...
def _compute_anchor_signal(series: list[tuple[date, float]]) -> RegimeSignal:
    ...
    # V4 circuit breaker: VTI drawdown > 15% forces RISK_OFF
    if drawdown_63d <= -0.15:
        regime = "RISK_OFF"
        crash_mode = True
...
def _risk_on_targets(
    *,
    ...
    crash_scale = 0.65 if signal.crash_mode else 1.0
    risk_budget = min(0.90, max(0.40, 0.90 * vol_scale * crash_scale))
    scaled = {symbol: weight * risk_budget for symbol, weight in weights.items()}
    # V4: 10% cash floor in RISK_ON
    cash = max(0.10, 1.0 - sum(scaled.values()))
...
def _transition_targets(
    *,
    ...
    if invested > 1.0:
        ...
        invested = sum(scaled.values())
    # V4: 15% cash floor in TRANSITION
    scaled[cash_symbol] = max(0.15, 1.0 - invested)
    return _weights_to_target_pct(scaled, cash_symbol=cash_symbol)
...
def _risk_off_targets(
    *,
    ...
    weights = _inverse_vol_weights(selected, feature_map)
    # V4: 70% defensive, 30% cash
    defensive_budget = 0.70
    scaled = {symbol: weight * defensive_budget for symbol, weight in weights.items()}
    scaled[cash_symbol] = max(0.30, 1.0 - sum(scaled.values()))
    return _weights_to_target_pct(scaled, cash_symbol=cash_symbol)
...
def _build_ticket_message(
    *,
    ...
    lines = [
        "RAEC 401(k) Global Macro Rebalance Ticket",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run RAEC 401(k) global macro manual strategy.")
