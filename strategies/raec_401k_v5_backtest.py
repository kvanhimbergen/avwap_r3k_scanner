"""Historical backtest for RAEC 401(k) V5 strategy."""
...
from strategies import raec_401k_v5
...
def run_backtest(
    *,
    ...
    start = raec_401k_v5._parse_date(start_date)
    end = raec_401k_v5._parse_date(end_date)
    ...
    all_symbols = list(raec_401k_v5.DEFAULT_UNIVERSE) + ["VTI", "QQQ"]
    ...
    for day in trading_days:
        ...
        if not last_eval_date:
            ...
            prior = raec_401k_v5._parse_date(last_eval_date)
            should_eval = prior != day

        if not should_eval:
            continue

        # 3. Run strategy logic (dual anchor)
        asof_str = day.strftime("%Y-%m-%d")
        try:
            vti_series = raec_401k_v5._sorted_series(
                provider.get_daily_close_series("VTI"), asof=day
            )
            qqq_series = raec_401k_v5._sorted_series(
        ...
        cash_symbol = "BIL"
        feature_map = raec_401k_v5._load_symbol_features(
            provider=provider, asof=day, cash_symbol=cash_symbol
        )
        targets = raec_401k_v5._targets_for_regime(
        ...
        if not last_eval_date or last_regime != signal.regime:
            ...
            drift = raec_401k_v5._compute_drift(allocations, targets)
            if any(abs(d) > raec_401k_v5.DRIFT_THRESHOLD_PCT for d in drift.values()):
                do_rebalance = True
            else:
        ...
        if do_rebalance:
            ...
            deltas = {s: d for s, d in deltas.items() if abs(d) >= raec_401k_v5.MIN_TRADE_PCT}
            capped = raec_401k_v5._apply_turnover_cap(
                deltas, max_weekly_turnover=raec_401k_v5.MAX_WEEKLY_TURNOVER_PCT
            )

            new_allocs = dict(allocations)
            for sym, delta in capped.items():
                if abs(delta) >= raec_401k_v5.MIN_TRADE_PCT:
    ...
    print("=" * 65)
    print("  RAEC 401(k) V5 — AI/Future Tech Backtest Results")
    print("=" * 65)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest RAEC 401(k) V5 strategy.")
