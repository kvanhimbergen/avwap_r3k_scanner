...
def _compute_anchor_signal(series: list[tuple[date, float]]) -> RegimeSignal:
    ...
    # V3 circuit breaker: VTI drawdown > 15% forces RISK_OFF
...
def _risk_on_targets(
    *,
    ...
    crash_scale = 0.60 if signal.crash_mode else 1.0
...
def _transition_targets(
    *,
    ...
    # Top 2 defensive by momentum (not static like V2)
...
def _build_ticket_message(
    *,
    ...
    lines = [
        "RAEC 401(k) Aggressive Rebalance Ticket",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run RAEC 401(k) aggressive manual strategy.")
