...
def run_backtest(
    *,
    ...
    all_symbols = list(raec_401k_v3.DEFAULT_UNIVERSE) + ["VTI"]
    ...
    for day in trading_days:
        ...
        if not last_eval_date:
            ...
            prior = raec_401k_v3._parse_date(last_eval_date)
        ...
        try:
            vti_series = raec_401k_v3._sorted_series(
        ...
        cash_symbol = "BIL"
        feature_map = raec_401k_v3._load_symbol_features(
            provider=provider, asof=day, cash_symbol=cash_symbol
        )
        targets = raec_401k_v3._targets_for_regime(
        ...
        if not last_eval_date or last_regime != signal.regime:
            ...
            drift = raec_401k_v3._compute_drift(allocations, targets)
            if any(abs(d) > raec_401k_v3.DRIFT_THRESHOLD_PCT for d in drift.values()):
                do_rebalance = True
            else:
        ...
        if do_rebalance:
            ...
            deltas = {s: d for s, d in deltas.items() if abs(d) >= raec_401k_v3.MIN_TRADE_PCT}
            capped = raec_401k_v3._apply_turnover_cap(
                deltas, max_weekly_turnover=raec_401k_v3.MAX_WEEKLY_TURNOVER_PCT
            )

            new_allocs = dict(allocations)
            for sym, delta in capped.items():
                if abs(delta) >= raec_401k_v3.MIN_TRADE_PCT:
    ...
    print("=" * 65)
    print("  RAEC 401(k) V3 — Historical Backtest Results")
    print("=" * 65)
