...
def _compute_single_anchor(series: list[tuple[date, float]]) -> dict:
...
def _compute_anchor_signal(
    ...
    vti_sig = _compute_single_anchor(vti_series)
    qqq_sig = _compute_single_anchor(qqq_series)
...
def _risk_on_targets(
    *,
    ...
    crash_scale = 0.50 if signal.crash_mode else 1.0
    risk_budget = min(1.0, max(0.40, 1.0 * vol_scale * crash_scale))
...
def _transition_targets(
    *,
    ...
    ranked_risk = _rank_symbols(RISK_UNIVERSE, feature_map, require_positive_momentum=True)
    selected_risk = ranked_risk[:2]
    scaled: dict[str, float] = {}
    if selected_risk:
        risk_weights = _inverse_vol_weights(selected_risk, feature_map)
    ...
    # Top 2 defensive by momentum
    defensive_candidates = [sym for sym in DEFENSIVE_UNIVERSE if sym != cash_symbol]
    ranked_defensive = _rank_symbols(defensive_candidates, feature_map, require_positive_momentum=False)
    selected_defensive = ranked_defensive[:2]
    if selected_defensive:
        defensive_weights = _inverse_vol_weights(selected_defensive, feature_map)
        for symbol, weight in defensive_weights.items():
            scaled[symbol] = scaled.get(symbol, 0.0) + (weight * 0.25)
    ...
    if invested > 1.0:
        ...
        invested = sum(scaled.values())
    scaled[cash_symbol] = max(0.05, 1.0 - invested)
    return _weights_to_target_pct(scaled, cash_symbol=cash_symbol)
...
def _risk_off_targets(
    *,
    ...
    scaled[cash_symbol] = max(0.20, 1.0 - sum(scaled.values()))
    return _weights_to_target_pct(scaled, cash_symbol=cash_symbol)
...
def _build_ticket_message(
    *,
    ...
    lines = [
        "RAEC 401(k) AI/Future Tech Rebalance Ticket",
...
def run_strategy(
    *,
    ...
    qqq_series = _sorted_series(provider.get_daily_close_series("QQQ"), asof=asof)
    signal = _compute_anchor_signal(vti_series, qqq_series)
    ...
    ranked_snapshot = _rank_symbols(RISK_UNIVERSE, feature_map, require_positive_momentum=False)[:5]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run RAEC 401(k) AI/future tech manual strategy.")
